using LibraryDifferentialEquationKepler25feb2024;
...
namespace WinFormsKeplerCrudeHalf27feb2024
{
...
Reference: Numerical methods for ODEs, Butcher(2008)
           */
            ...
            Text = "Sophisticated Runge-Kutta versus crude Runge-Kutta for Kepler's elliptic planetary motion (using half precision floating points)";
            ...
            var kepler = new DifferentialEquationsKeplerHalf();
            int numberOfFirstOrderEquations = kepler.NumberOfFirstOrderEquations;
            ISolver26feb2024<Half> solver1 = new DifferentialEquationsSolver26feb2024<Half>(kepler, Method.RK41 | Method.Sophisticated);
            ISolver26feb2024<Half> solver2 = new DifferentialEquationsSolver26feb2024<Half>(kepler, Method.RK41 | Method.Crude);

            double interval = Math.PI;

            ulong number_of_steps = 1;
            ...
            for (int k = 0; k < kmax; k++)
            {
                ...
                solver1.Solve(interval: (Half)interval, x_end: (Half)x_end, initialCondition: ic, number_of_steps: number_of_steps, delta_x: out Half delta_x, out NumericalSolution26feb2024<Half> solutionSophisticated);
                solver2.Solve(interval: (Half)interval, x_end: (Half)x_end, initialCondition: ic, number_of_steps: number_of_steps, delta_x: out Half delta_x_crude, out NumericalSolution26feb2024<Half> solutionCrude);
                ...
                {
                    y_sophisticated[i] = ((double)solutionSophisticated.Y[i]);
                }

                double[] y_crude = new double[numberOfFirstOrderEquations];
                for (int i = 0; i < numberOfFirstOrderEquations; i++)
                {
                    y_crude[i] = ((double)solutionCrude.Y[i]);
                }
                ...
                series1.Points.Add(new DataPoint(Math.Log10((double)delta_x), Math.Log10(abs(error_sophisticated))));
                series2.Points.Add(new DataPoint(Math.Log10((double)delta_x), Math.Log10(abs(error_crude))));
